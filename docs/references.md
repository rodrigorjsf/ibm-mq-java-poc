...
then deduplicated: recurring links (the `com.ibm.mq.jakarta.client` jar, the `setgetweb` report
pages) are listed once with the union of the concepts they grounded. The shipped client is
`com.ibm.mq.jakarta.client` 9.4.5.0 (Jakarta Messaging 3.0 / `jakarta.jms`, ADR-0012); the
`com.ibm.mq.allclient` (javax.jms / JMS 2.0) sources are retained as a labeled migration
reference.
...
|---|---|---|
| `com.ibm.mq.jakarta.client` 9.4.5.0 jar (bytecode — **SHIPPED / PRIMARY client**, ADR-0012) | The `jakarta.jms` (Jakarta Messaging 3.0) client the project ships (pinned 9.4.5.0 for broker-image + fact-sheet parity — a namespace-only change, not a version bump). Bytecode re-verification per the fact sheet's *Jakarta re-verification (9.4.5.0)*: exactly three FQCNs relocate — `WMQConstants` → `com.ibm.msg.client.jakarta.wmq.WMQConstants`, `MQConnectionFactory` → `com.ibm.mq.jakarta.jms.MQConnectionFactory`, `JmsConstants` → `com.ibm.msg.client.jakarta.jms.JmsConstants`; every value is unchanged — the `MQRO_*` / `MQFB_*` integers (`MQFB_COA=259`, `MQFB_COD=260`) stay in `com.ibm.mq.constants.CMQC` / `MQConstants`, and all `JMS_IBM_*` / `XMSC_*` / `WMQ_*` String values are identical. Grounds the jakarta-primary stack. | <https://repo1.maven.org/maven2/com/ibm/mq/com.ibm.mq.jakarta.client/9.4.5.0/com.ibm.mq.jakarta.client-9.4.5.0.jar> |
| `com.ibm.mq.allclient` 9.4.5.0 jar (bytecode, sha1 `26c8f5cd163847990d270acf2f4f0a7f773c3bbd`) — **legacy / migration reference** | Pre-migration `javax.jms` (JMS 2.0) client. Authoritative values for the report property fields (`JMS_IBM_REPORT_*` → mixed-case String values, **identical under jakarta**), the `MQRO_*` / `MQFB_*` integer constants (declared in `com.ibm.mq.constants.CMQC`, **not** `WMQConstants` — unchanged under jakarta), the connection properties (`WMQ_*` → `XMSC_*`), and the auth/TLS constants. Refuted `WMQ_USER_AUTHENTICATION_MQCSP` (correct field is `USER_AUTHENTICATION_MQCSP`) and `WMQ_SSL_CERT_STORES` (only `_COL` / `_STR` exist); confirmed the `WMQ_APPLICATIONNAME` vs `XMSC_WMQ_APPNAME` casing divergence. Kept as the source for the javax→jakarta migration section. | <https://repo1.maven.org/maven2/com/ibm/mq/com.ibm.mq.allclient/9.4.5.0/com.ibm.mq.allclient-9.4.5.0.jar> |
| `com.ibm.mq.allclient` Maven metadata — **legacy / migration reference** | Artifact existence / version line for the pre-migration javax client: 9.4.5.0 jar size (8,477,900 bytes), publish date (2026-01-30), `<latest>`/`<release>` = 9.4.5.1; `javax.jms` / JMS 2.0 client (POM depends on `javax.jms:javax.jms-api:2.0.1`). | <https://repo1.maven.org/maven2/com/ibm/mq/com.ibm.mq.allclient/maven-metadata.xml> |
...
| `io.micronaut.jms:micronaut-jms-core` 4.3.0 jar | The Micronaut JMS module is **jakarta-only** (bytecode 4.3.0 references only `jakarta/jms`), so it is jakarta-aligned with the migrated stack (it was excluded back when the project was javax); the real coordinate is `micronaut-jms-core` (+ `micronaut-jms-bom`), there is no bare `micronaut-jms` artifact. | <https://repo1.maven.org/maven2/io/micronaut/jms/micronaut-jms-core/4.3.0/micronaut-jms-core-4.3.0.jar> |
| `org.messaginghub:pooled-jms` `JmsPoolConnectionFactory` source (1.2.8) | Framing for the pool's namespace-by-version split: `pooled-jms` 1.x **and** 2.x are both `javax.jms`; **3.x is `jakarta.jms` — the SHIPPED line (`3.2.2`)**, paired with `com.ibm.mq.jakarta.client`. The FQ class `org.messaginghub.pooled.jms.JmsPoolConnectionFactory` and its package are **identical across 2.x and 3.x** — only the version + bound JMS namespace change. | <https://raw.githubusercontent.com/messaginghub/pooled-jms/1.2.8/pooled-jms/src/main/java/org/messaginghub/pooled/jms/JmsPoolConnectionFactory.java> |
| `org.messaginghub:pooled-jms` **2.0.9 sources jar** (`JmsPoolConnectionFactory.java`, `pool/PooledConnection.java`) — **legacy / migration reference** | Authentic tuning defaults grounding the pooling deep dive (the **package + default values are identical in the shipped 3.2.2**, so this remains the authoritative source for them): `DEFAULT_MAX_CONNECTIONS = 1`, `DEFAULT_MAX_SESSIONS_PER_CONNECTION = 500`, `blockIfSessionPoolIsFull = true` + timeout `-1` (block forever on exhaustion), `connectionIdleTimeout = 30 000 ms`, eviction off (`-1`); the two-ceiling model (`maxConnections × maxSessionsPerConnection`) and `setBlockWhenExhausted` wiring. Persisted in `research-output/pooled-jms-factory-tuning.md`. | local `~/.m2/repository/org/messaginghub/pooled-jms/2.0.9/pooled-jms-2.0.9-sources.jar` (Maven Central `org.messaginghub:pooled-jms:2.0.9`) |
...
| Source | Grounds (concept / term / discovery) | Link |
|---|---|---|
| IBM MQ 9.4 — Java application dev via Maven | The Maven-repository consumption model; confirms `com.ibm.mq.allclient.jar` is the `javax.jms` / JMS 2.0 client (legacy / migration reference) and that `com.ibm.mq.jakarta.client` is its `jakarta.jms` / Jakarta Messaging 3.0 sibling — the **shipped** client (ADR-0012). | <https://www.ibm.com/docs/en/ibm-mq/9.4.x?topic=applications-java-application-development-using-maven-repository> |
| IBM MQ 9.4 — Developing JMS/Jakarta Messaging Java apps | The dedicated **Java 25** operational guidance (refuting "Java 25 not listed for 9.4.5"): `TLS_RSA_*` CipherSpecs disabled from Java 25, native-access warnings (the warning applies equally to the shipped `com.ibm.mq.jakarta.client.jar` and the legacy `com.ibm.mq.allclient.jar` — both load the same native library) → run with `--enable-native-access=ALL-UNNAMED`. | <https://www.ibm.com/docs/en/ibm-mq/9.4.x?topic=applications-developing-jmsjakarta-messaging-java> |
